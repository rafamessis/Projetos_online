...
for index, value in enumerate(links):
    if value in links:
        with(open('/home/rafael/Projetos_Python/Projetos_Online/RMS_solution/Arquivos/links.txt','a+')) as f:
            f.write(value+'\n')
            f.close()                        
            #print(value)
...
def extrair_conteudo(content):
    urls_visitadas = set()
    urls_validas = set()
    arq = open('/home/rafael/Projetos_Python/Projetos_Online/RMS_solution/Arquivos/links.txt', 'r')
    texto = arq.readlines()
    ...
    for linha in texto:
        if linha not in urls_visitadas:
            for index, linha in enumerate(texto):
                ...
                with(open('/home/rafael/Projetos_Python/Projetos_Online/RMS_solution/Arquivos/correlation.txt','a+')) as filecorrelation:
                    filecorrelation.write(str(index)+ '  '+ linha+'\n')
                    filecorrelation.close()                        
                    
                
                    filename ='/home/rafael/Projetos_Python/Projetos_Online/RMS_solution/Arquivos/Keys/' + str(index)+' .keys'
